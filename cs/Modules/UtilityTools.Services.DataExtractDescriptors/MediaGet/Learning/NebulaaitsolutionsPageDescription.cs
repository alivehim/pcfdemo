...
{
    public class NebulaaitsolutionsPageDescription : BaseMediaPageDescription, IGrabArtcleContent, IMediaPageDescriptor<MediaDataDescription>
    {
        private string XPATH = @"//*[@class='blog clearfix']/li";
        private string CONTENTXPATH = @"//*[@class='post-inner']/article";
        public string ShortIcon => "website.png";

        private readonly IMediaSymbolDBService mediaSymbolDBService;
        private readonly IEventAggregator eventAggregator;

        public NebulaaitsolutionsPageDescription(IMediaSymbolDBService mediaSymbolDBService, IEventAggregator eventAggregator)
        {
        ...
        protected async override Task<IExtractResult<MediaDataDescription>> ProcessAsync()
        {
            ...
            if (!string.IsNullOrEmpty(html))
            {
                ...
                foreach (HtmlNode categoryNode in categoryNodeList)
                {
                    ...
                    var innerNode = categoryNode.SelectSingleNode(@".//h3/a");
                    ...
                    data.Add(new MediaDataDescription
                    {
                        Name = name,
                        Url = url,
                        MediaType = MediaSymbolType.Nebulaaitsolutions,
                        Order = index
                    });
                    index++;
                }
