...
{
    public class XRMtricksDataDescription : BaseMediaPageDescription, IGrabArtcleContent, IMediaPageDescriptor<MediaDataDescription>
    {
        ...
        public XRMtricksDataDescription(IMediaSymbolDBService mediaSymbolDBService, IEventAggregator eventAggregator)
        {
        ...
        protected async override Task<IExtractResult<MediaDataDescription>> ProcessAsync()
        {
            ...
            //string html = HttpHelper.GetUrlContentAsync(MediaGetContext.Key, Encoding.GetEncoding("utf-8")).GetAwaiter().GetResult();
            ...
            if (!string.IsNullOrEmpty(html))
            {
                ...
                foreach (HtmlNode categoryNode in categoryNodeList)
                {
                    ...
                    data.Add(new MediaDataDescription
                    {
                        Name = name,
                        Url = url,
                        MediaType = MediaSymbolType.XRMtricks
                    });
                    index++;
                }
            }
