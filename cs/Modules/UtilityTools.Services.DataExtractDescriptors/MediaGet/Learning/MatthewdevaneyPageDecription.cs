...
{
    public class MatthewdevaneyPageDecription : BaseMediaPageDescription, IGrabArtcleContent, IMediaPageDescriptor<MediaDataDescription>
    {
        private string XPATH = @"//*[@class='post-title']";
        private string CONTENTXPATH = @"//*[@id='main']/section/article";
        public string ShortIcon => "website.png";

        private readonly IMediaSymbolDBService mediaSymbolDBService;
        private readonly IEventAggregator eventAggregator;

        public MatthewdevaneyPageDecription(IMediaSymbolDBService mediaSymbolDBService, IEventAggregator eventAggregator)
        {
            ...
            if (!string.IsNullOrEmpty(html))
            {
                ...
                foreach (HtmlNode categoryNode in categoryNodeList)
                {
                    ...
                    data.Add(new MediaDataDescription
                    {
                        Name = name,
                        Url = url,
                        MediaType = MediaSymbolType.Matthewdevaney,
                        Order = index
                    });
                    index++;
                }
