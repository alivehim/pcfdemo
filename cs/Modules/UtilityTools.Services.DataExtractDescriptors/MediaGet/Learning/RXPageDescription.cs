...
{
    public class RXPageDescription : BaseMediaPageDescription, IGrabArtcleContent, IMediaPageDescriptor<MediaDataDescription>
    {
        private string XPATH = @"//li/a";
        private string CONTENTXPATH = @"//*[@class='main']";
        public string ShortIcon => "website.png";

        private readonly IMediaSymbolDBService mediaSymbolDBService;
        private readonly IEventAggregator eventAggregator;

        public RXPageDescription(IMediaSymbolDBService mediaSymbolDBService, IEventAggregator eventAggregator)
        {
        ...
        protected async override Task<IExtractResult<MediaDataDescription>> ProcessAsync()
        {
            ...
            if (!string.IsNullOrEmpty(html))
            {
                ...
                var symbol = mediaSymbolDBService.GetMediaSymbol(MediaSymbolType.Rx.ToString());
                ...
                foreach (HtmlNode categoryNode in categoryNodeList)
                {
                    ...
                    var url = $"{symbol.Address}/Content/v1.0.10621.0/{categoryNode.GetAttributeValue("href", string.Empty)}";
                    var name = HttpUtility.HtmlDecode(ContentReplace(categoryNode.InnerText).Replace(".", ""));

                    data.Add(new MediaDataDescription
                    {
                        Name = name,
                        Url = url,
                        MediaType = MediaSymbolType.Rx,
                        Order = index
                    });
                    index++;
                }
        ...
        public async Task<string> GetContentAsync(string address)
        {
            ...
            content = Regex.Replace(content, @"<div style=""float: right; margin: 10px 0px 10px 10px; width: 140px; font-size: 11px;(.*)</div>", (mc) => { return string.Empty; });
