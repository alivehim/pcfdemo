...
{
    public class EloquentjavascriptPageDescription : BaseMediaPageDescription, IGrabArtcleContent, IMediaPageDescriptor<MediaDataDescription>
    {
        private string XPATH = @"//*[@class='toc']/li";
        private string CONTENTXPATH = @"//article";
        public string ShortIcon => "website.png";

        private readonly IMediaSymbolDBService mediaSymbolDBService;
        private readonly IEventAggregator eventAggregator;

        public EloquentjavascriptPageDescription(IMediaSymbolDBService mediaSymbolDBService, IEventAggregator eventAggregator)
        {
        ...
        protected async override Task<IExtractResult<MediaDataDescription>> ProcessAsync()
        {
            ...
            if (!string.IsNullOrEmpty(html))
            {
                ...
                var symbol = mediaSymbolDBService.GetMediaSymbol(MediaSymbolType.Eloquentjavascript.ToString());
                ...
                foreach (HtmlNode categoryNode in categoryNodeList)
                {
                    ...
                    data.Add(new MediaDataDescription
                    {
                        Name = name,
                        Url = url,
                        MediaType = MediaSymbolType.Eloquentjavascript,
                        Order = index
                    });
                    index++;
                }
