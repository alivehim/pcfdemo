...
{
    public class HCLTechPageDescription : BaseMediaPageDescription, IGrabArtcleContent, IMediaPageDescriptor<MediaDataDescription>
    {
        private string XPATH = @"//*[@class='post-wrap']";
        private string CONTENTXPATH = @"//*[@id='text_block-773-41']";
        public string ShortIcon => "website.png";

        private readonly IMediaSymbolDBService mediaSymbolDBService;
        private readonly IEventAggregator eventAggregator;

        public HCLTechPageDescription(IMediaSymbolDBService mediaSymbolDBService, IEventAggregator eventAggregator)
        {
            ...
            if (!string.IsNullOrEmpty(html))
            {
                ...
                foreach (HtmlNode categoryNode in categoryNodeList)
                {
                    ...
                    data.Add(new MediaDataDescription
                    {
                        Name = name,
                        Url = url,
                        MediaType = MediaSymbolType.HCLTech,
                        Order = index
                    });
                    index++;
                }
