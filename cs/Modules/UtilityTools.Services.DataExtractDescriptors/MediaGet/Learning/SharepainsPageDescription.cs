...
{
    public class SharepainsPageDescription : BaseMediaPageDescription, IGrabArtcleContent, IMediaPageDescriptor<MediaDataDescription>
    {
        private string XPATH = @"//*[@class='small']";
        private string CONTENTXPATH = @"//*[@class='mg-blog-post-box']";
        public string ShortIcon => "website.png";

        private readonly IMediaSymbolDBService mediaSymbolDBService;
        private readonly IEventAggregator eventAggregator;

        public SharepainsPageDescription(IMediaSymbolDBService mediaSymbolDBService, IEventAggregator eventAggregator)
        {
        ...
        protected async override Task<IExtractResult<MediaDataDescription>> ProcessAsync()
        {
            ...
            if (!string.IsNullOrEmpty(html))
            {
                ...
                foreach (HtmlNode categoryNode in categoryNodeList)
                {
                    ...
                    var innerNode = categoryNode.SelectSingleNode(@".//h4/a");
                    ...
                    data.Add(new MediaDataDescription
                    {
                        Name = name,
                        Url = url,
                        MediaType = MediaSymbolType.Sharepains,
                        Order = index
                    });
                    index++;
                }
