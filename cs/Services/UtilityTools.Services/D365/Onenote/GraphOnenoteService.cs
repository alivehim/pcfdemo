...
{
    public class GraphOnenoteService : IGraphOnenoteService
    {

        [LogAspect("GetOnenoteBooks")]
        public async Task<IList<BookItem>> GetOnenoteBooksAsync()
        {
            string address = "https://graph.microsoft.com/v1.0/me/onenote/notebooks";

            var result = await HttpHelper.GetDynamic365ContentAsync(address, Encoding.UTF8, Settings.Current.GraphToken);
        ...
        public async Task<IList<SectionItem>> GetOnenoteSectionsAsync(string url)
        {
            ...
            var result = await HttpHelper.GetDynamic365ContentAsync(url, Encoding.UTF8, Settings.Current.GraphToken);
        ...
        public async Task<SectionItem> CreateSectionAsync(string url, string sectionName)
        {
            ...
            var result = await HttpHelper.PostDataToUrl(url, json, Settings.Current.GraphToken);
...
<html>
  ...
  <body>
    {content.Replace("’", "'")}
            ...
            await HttpHelper.PostDataToUrlWithText(sectionUrl, html, Settings.Current.GraphToken);
        }
    }
}
