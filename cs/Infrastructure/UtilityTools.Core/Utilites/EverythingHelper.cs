...
namespace UtilityTools.Core.Utilites
{
    public class EverythingHelper
    {
        ...
        public static IList<EverythingResult> Search(string fileName)
        {
            UInt32 i;
            var list = new List<EverythingResult>();
            ...
            for (i = 0; i < (count < 50 ? count : 50); i++)
            {
                ...
                Everything_GetResultFullPathName(i, sb, (uint)sb.Length);

                list.Add(new EverythingResult
                {
                    Size = size,
